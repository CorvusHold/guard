...
func TestMagic_Flow_Integration(t *testing.T) {
	...
	if fe.lastBody == "" {
		t.Fatalf("expected email body captured")
	}
	...
	if err != nil {
		t.Fatalf("magic verify failed: %v", err)
	}
	...
	for _, e := range events {
		if e.Type == "auth.magic.login.success" {
			if e.Meta["provider"] != "magic" {
				t.Fatalf("provider mismatch: %v", e.Meta["provider"])
			}
			if e.Meta["email"] != email {
	...
	if !found {
		t.Fatalf("expected auth.magic.login.success event")
	}
