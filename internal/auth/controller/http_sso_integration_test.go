...
func TestHTTP_SSO_Dev_RedirectAllowlist_Disallowed_400(t *testing.T) {
    ...
    if !found { t.Fatalf("expected auth.sso.start.failure event with redirect_disallowed") }
}
...
func TestHTTP_SSO_Dev_RedirectAllowlist_AllowedPrefix_OK(t *testing.T) {
    ...
    for _, ev := range events {
        if ev.Type == "auth.sso.login.success" && ev.TenantID == tenantID {
            if ev.Meta["provider"] == "google" && ev.Meta["email"] == expectedEmail { okEvent = true }
        }
    }
    if !okEvent { t.Fatalf("expected auth.sso.login.success event for dev flow") }
}
