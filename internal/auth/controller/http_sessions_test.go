...
func TestHTTP_Sessions_List_And_Revoke(t *testing.T) {
    ...
    if lrec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", lrec.Code, lrec.Body.String()) }
    var toks tokensResponse
    if err := json.NewDecoder(bytes.NewReader(lrec.Body.Bytes())).Decode(&toks); err != nil { t.Fatalf("decode tokens: %v", err) }
    ...
    if err := json.NewDecoder(bytes.NewReader(rec200.Body.Bytes())).Decode(&sl); err != nil { t.Fatalf("decode sessions: %v", err) }
    if len(sl.Sessions) == 0 { t.Fatalf("expected at least 1 session") }

    // find a non-revoked session to revoke
    sid := sl.Sessions[0].ID
    for _, s := range sl.Sessions {
        if !s.Revoked { sid = s.ID; break }
    }
    ...
    if rrec401.Code != http.StatusUnauthorized { t.Fatalf("expected 401, got %d: %s", rrec401.Code, rrec401.Body.String()) }
    ...
    if rrec400.Code != http.StatusBadRequest { t.Fatalf("expected 400, got %d: %s", rrec400.Code, rrec400.Body.String()) }
    ...
    if rrec200.Code != http.StatusNoContent { t.Fatalf("expected 204, got %d: %s", rrec200.Code, rrec200.Body.String()) }
    ...
    if recList2.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", recList2.Code, recList2.Body.String()) }
    var sl2 sessionsListResp
    if err := json.NewDecoder(bytes.NewReader(recList2.Body.Bytes())).Decode(&sl2); err != nil { t.Fatalf("decode sessions2: %v", err) }
    found := false
    for _, s := range sl2.Sessions {
        if s.ID == sid { found = true; if !s.Revoked { t.Fatalf("expected session %s revoked after revoke", sid) } }
    }
    if !found { t.Fatalf("revoked session id not found in listing") }
}
