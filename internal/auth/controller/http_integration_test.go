...
func TestHTTP_Password_Signup_Login_Refresh_AuditAndClaims(t *testing.T) {
	...
	if iss, _ := sclaims["iss"].(string); iss != os.Getenv("PUBLIC_BASE_URL") {
		t.Fatalf("iss mismatch: %v", sclaims["iss"])
	}
	if aud, _ := sclaims["aud"].(string); aud != os.Getenv("PUBLIC_BASE_URL") {
		t.Fatalf("aud mismatch: %v", sclaims["aud"])
	}
	...
	if iss, _ := lclaims["iss"].(string); iss != os.Getenv("PUBLIC_BASE_URL") {
		t.Fatalf("iss mismatch: %v", lclaims["iss"])
	}
	if aud, _ := lclaims["aud"].(string); aud != os.Getenv("PUBLIC_BASE_URL") {
		t.Fatalf("aud mismatch: %v", lclaims["aud"])
	}
