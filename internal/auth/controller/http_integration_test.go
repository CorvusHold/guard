...
func TestHTTP_Authorize_AllowAndDeny(t *testing.T) {
    ...
    if err := auth.AddGroupMember(ctx, grp.ID, intr.UserID); err != nil { t.Fatalf("add member: %v", err) }
    ...
    areq := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(ab))
    ...
    var allowResp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(arec.Body.Bytes())).Decode(&allowResp); err != nil {
        t.Fatalf("decode allow resp: %v", err)
    }
    if !allowResp.Allowed { t.Fatalf("expected allowed=true, got: %s", arec.Body.String()) }
    ...
    ureq := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(ub))
    ureq.Header.Set("Content-Type", "application/json")
    ureq.Header.Set("Authorization", "Bearer "+toks.AccessToken)
    urec := httptest.NewRecorder()
    e.ServeHTTP(urec, ureq)
    if urec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", urec.Code, urec.Body.String()) }
    var uresp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(urec.Body.Bytes())).Decode(&uresp); err != nil { t.Fatalf("decode user resp: %v", err) }
    if !uresp.Allowed { t.Fatalf("expected allowed=true for explicit user, got: %s", urec.Body.String()) }
    ...
    greq := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(gb))
    ...
    if grec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", grec.Code, grec.Body.String()) }
    var gresp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(grec.Body.Bytes())).Decode(&gresp); err != nil { t.Fatalf("decode group resp: %v", err) }
    if !gresp.Allowed { t.Fatalf("expected allowed=true for group subject, got: %s", grec.Body.String()) }
    ...
    if mrec.Code != http.StatusBadRequest { t.Fatalf("expected 400 for missing subject_id, got %d: %s", mrec.Code, mrec.Body.String()) }
    ...
    ivreq := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(ivb))
    ...
    if ivrec.Code != http.StatusBadRequest { t.Fatalf("expected 400 for invalid subject_type, got %d: %s", ivrec.Code, ivrec.Body.String()) }
    ...
    dreq := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(dbb))
    ...
    var denyResp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(drec.Body.Bytes())).Decode(&denyResp); err != nil {
        t.Fatalf("decode deny resp: %v", err)
    }
    if denyResp.Allowed { t.Fatalf("expected allowed=false, got: %s", drec.Body.String()) }
    ...
    areq2 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(ab))
    ...
    if arec2.Code != http.StatusOK { t.Fatalf("expected 200 after tuple delete, got %d: %s", arec2.Code, arec2.Body.String()) }
    var allowResp2 struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(arec2.Body.Bytes())).Decode(&allowResp2); err != nil { t.Fatalf("decode allow2 resp: %v", err) }
    if allowResp2.Allowed { t.Fatalf("expected allowed=false after tuple delete, got: %s", arec2.Body.String()) }

    // Final cleanup: remove membership and delete group
    if err := auth.RemoveGroupMember(ctx, grp.ID, intr.UserID); err != nil { t.Fatalf("remove member: %v", err) }
    if err := auth.DeleteGroup(ctx, grp.ID, tenantID); err != nil { t.Fatalf("delete group: %v", err) }
}
...
func TestHTTP_Authorize_ObjectScopeAndCrossTenant(t *testing.T) {
    ...
    tr := trepo.New(pool)
    tenantA := uuid.New()
    if err := tr.Create(ctx, tenantA, "http-authz-obj-"+tenantA.String()); err != nil { t.Fatalf("create tenantA: %v", err) }
    tenantB := uuid.New()
    if err := tr.Create(ctx, tenantB, "http-authz-obj-"+tenantB.String()); err != nil { t.Fatalf("create tenantB: %v", err) }
    time.Sleep(25 * time.Millisecond)
    ...
    srec := httptest.NewRecorder()
    e.ServeHTTP(srec, sreq)
    if srec.Code != http.StatusCreated { t.Fatalf("expected 201, got %d: %s", srec.Code, srec.Body.String()) }
    var toks tokensResponse
    ...
    // Resolve user id from token
    intr, err := auth.Introspect(ctx, toks.AccessToken)
    if err != nil || !intr.Active { t.Fatalf("introspect failed: %v", err) }
    ...
    a1req := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(a1b))
    ...
    if a1rec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", a1rec.Code, a1rec.Body.String()) }
    var a1resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a1rec.Body.Bytes())).Decode(&a1resp); err != nil { t.Fatalf("decode a1: %v", err) }
    if !a1resp.Allowed { t.Fatalf("expected allowed=true for object-specific grant, got: %s", a1rec.Body.String()) }
    ...
    a2req := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(a2b))
    ...
    if a2rec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", a2rec.Code, a2rec.Body.String()) }
    var a2resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a2rec.Body.Bytes())).Decode(&a2resp); err != nil { t.Fatalf("decode a2: %v", err) }
    if a2resp.Allowed { t.Fatalf("expected allowed=false for different object_id, got: %s", a2rec.Body.String()) }
    ...
    a3req := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(a3b))
    ...
    if a3rec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", a3rec.Code, a3rec.Body.String()) }
    var a3resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a3rec.Body.Bytes())).Decode(&a3resp); err != nil { t.Fatalf("decode a3: %v", err) }
    if a3resp.Allowed { t.Fatalf("expected allowed=false for missing object_id with only object-specific grant, got: %s", a3rec.Body.String()) }
    ...
    a4req := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(a4b))
    ...
    if a4rec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", a4rec.Code, a4rec.Body.String()) }
    var a4resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a4rec.Body.Bytes())).Decode(&a4resp); err != nil { t.Fatalf("decode a4: %v", err) }
    if !a4resp.Allowed { t.Fatalf("expected allowed=true for type-scoped grant, got: %s", a4rec.Body.String()) }
    ...
    ctreq := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(ctb))
    ...
    if ctrec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", ctrec.Code, ctrec.Body.String()) }
    var ctresp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(ctrec.Body.Bytes())).Decode(&ctresp); err != nil { t.Fatalf("decode cross-tenant: %v", err) }
    if ctresp.Allowed { t.Fatalf("expected allowed=false for cross-tenant check, got: %s", ctrec.Body.String()) }
...
func TestHTTP_Authorize_Group_ObjectAndTypeScope(t *testing.T) {
    ...
    tr := trepo.New(pool)
    tenantID := uuid.New()
    if err := tr.Create(ctx, tenantID, "http-authz-group-"+tenantID.String()); err != nil { t.Fatalf("create tenant: %v", err) }
    time.Sleep(25 * time.Millisecond)
    ...
    var a1resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a1rec.Body.Bytes())).Decode(&a1resp); err != nil { t.Fatalf("decode a1: %v", err) }
    if !a1resp.Allowed { t.Fatalf("expected allowed=true for group object-scoped grant") }
    ...
    var a2resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a2rec.Body.Bytes())).Decode(&a2resp); err != nil { t.Fatalf("decode a2: %v", err) }
    if a2resp.Allowed { t.Fatalf("expected allowed=false for different object_id") }
    ...
    var a3resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a3rec.Body.Bytes())).Decode(&a3resp); err != nil { t.Fatalf("decode a3: %v", err) }
    if a3resp.Allowed { t.Fatalf("expected allowed=false when missing object_id with only object-scoped grant") }

    // Cleanup object-specific grant
    if err := auth.DeleteACLTuple(ctx, tenantID, "group", grp.ID, perm, otype, &oid); err != nil { t.Fatalf("delete group object-scoped acl: %v", err) }
    ...
    var a4resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a4rec.Body.Bytes())).Decode(&a4resp); err != nil { t.Fatalf("decode a4: %v", err) }
    if !a4resp.Allowed { t.Fatalf("expected allowed=true for group type-scoped grant") }
    ...
    a5req := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(a5b))
    ...
    if a5rec.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", a5rec.Code, a5rec.Body.String()) }
    var a5resp struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(a5rec.Body.Bytes())).Decode(&a5resp); err != nil { t.Fatalf("decode a5: %v", err) }
    if !a5resp.Allowed { t.Fatalf("expected allowed=true for group type-scoped grant without object_id") }

    // Cleanup
    if err := auth.DeleteACLTuple(ctx, tenantID, "group", grp.ID, perm, otype, nil); err != nil { t.Fatalf("delete group type-scoped acl: %v", err) }
    if err := auth.DeleteGroup(ctx, grp.ID, tenantID); err != nil { t.Fatalf("delete group: %v", err) }
}
...
func TestHTTP_Authorize_WildcardObjectType(t *testing.T) {
    ...
    tr := trepo.New(pool)
    tenantID := uuid.New()
    if err := tr.Create(ctx, tenantID, "http-authz-wildcard-"+tenantID.String()); err != nil { t.Fatalf("create tenant: %v", err) }
    time.Sleep(25 * time.Millisecond)
    ...
    for _, ot := range []string{"report", "invoice", "tenant"} {
        ...
        req := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(bb))
        req.Header.Set("Content-Type", "application/json")
        req.Header.Set("Authorization", "Bearer "+toks.AccessToken)
        rec := httptest.NewRecorder()
        e.ServeHTTP(rec, req)
        if rec.Code != http.StatusOK { t.Fatalf("expected 200 for %s, got %d: %s", ot, rec.Code, rec.Body.String()) }
        var rsp struct{ Allowed bool `json:"allowed"` }
        if err := json.NewDecoder(bytes.NewReader(rec.Body.Bytes())).Decode(&rsp); err != nil { t.Fatalf("decode resp: %v", err) }
        if !rsp.Allowed { t.Fatalf("expected allowed=true for wildcard on %s", ot) }
    }
    ...
    req2 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(b2))
    req2.Header.Set("Content-Type", "application/json")
    req2.Header.Set("Authorization", "Bearer "+toks.AccessToken)
    rec2 := httptest.NewRecorder()
    e.ServeHTTP(rec2, req2)
    if rec2.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", rec2.Code, rec2.Body.String()) }
    var rsp2 struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(rec2.Body.Bytes())).Decode(&rsp2); err != nil { t.Fatalf("decode resp2: %v", err) }
    if !rsp2.Allowed { t.Fatalf("expected allowed=true for wildcard without object_id") }

    // Cleanup
    if err := auth.DeleteACLTuple(ctx, tenantID, "user", intr.UserID, perm, "*", nil); err != nil { t.Fatalf("delete wildcard acl: %v", err) }
}
...
func TestHTTP_Authorize_Negative_InvalidInputs(t *testing.T) {
    ...
    tr := trepo.New(pool)
    tenantID := uuid.New()
    if err := tr.Create(ctx, tenantID, "http-authz-neg-"+tenantID.String()); err != nil { t.Fatalf("create tenant: %v", err) }
    time.Sleep(25 * time.Millisecond)
    ...
    req1 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(b1))
    req1.Header.Set("Content-Type", "application/json")
    req1.Header.Set("Authorization", "Bearer "+toks.AccessToken)
    rec1 := httptest.NewRecorder()
    e.ServeHTTP(rec1, req1)
    if rec1.Code != http.StatusBadRequest { t.Fatalf("expected 400 for invalid tenant_id, got %d: %s", rec1.Code, rec1.Body.String()) }
    ...
    rec2 := httptest.NewRecorder()
    e.ServeHTTP(rec2, req2)
    if rec2.Code != http.StatusBadRequest { t.Fatalf("expected 400 for invalid subject_id, got %d: %s", rec2.Code, rec2.Body.String()) }
    ...
    req3 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(b3))
    req3.Header.Set("Content-Type", "application/json")
    req3.Header.Set("Authorization", "Bearer "+toks.AccessToken)
    rec3 := httptest.NewRecorder()
    e.ServeHTTP(rec3, req3)
    if rec3.Code != http.StatusBadRequest { t.Fatalf("expected 400 for missing object_type, got %d: %s", rec3.Code, rec3.Body.String()) }
}
...
func TestHTTP_Authorize_CombinedGrants(t *testing.T) {
    ...
    tr := trepo.New(pool)
    tenantID := uuid.New()
    if err := tr.Create(ctx, tenantID, "http-authz-combined-"+tenantID.String()); err != nil { t.Fatalf("create tenant: %v", err) }
    time.Sleep(25 * time.Millisecond)
    ...
    oid2 := uuid.New().String()
    // object-specific
    if _, err := auth.CreateACLTuple(ctx, tenantID, "user", intr.UserID, perm, otype, &oid1, nil); err != nil { t.Fatalf("create obj-specific: %v", err) }
    // type-scoped
    if _, err := auth.CreateACLTuple(ctx, tenantID, "user", intr.UserID, perm, otype, nil, nil); err != nil { t.Fatalf("create type-scoped: %v", err) }
    ...
    r1 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(b1))
    r1.Header.Set("Content-Type", "application/json")
    r1.Header.Set("Authorization", "Bearer "+toks.AccessToken)
    w1 := httptest.NewRecorder()
    e.ServeHTTP(w1, r1)
    if w1.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", w1.Code, w1.Body.String()) }
    var p1 struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(w1.Body.Bytes())).Decode(&p1); err != nil { t.Fatalf("decode p1: %v", err) }
    if !p1.Allowed { t.Fatalf("expected allowed=true for combined grants (obj1)") }
    ...
    r2 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(b2))
    ...
    var p2 struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(w2.Body.Bytes())).Decode(&p2); err != nil { t.Fatalf("decode p2: %v", err) }
    if !p2.Allowed { t.Fatalf("expected allowed=true for combined grants (obj2)") }
    ...
    r3 := httptest.NewRequest(http.MethodPost, "/v1/auth/authorize", bytes.NewReader(b3))
    r3.Header.Set("Content-Type", "application/json")
    r3.Header.Set("Authorization", "Bearer "+toks.AccessToken)
    w3 := httptest.NewRecorder()
    e.ServeHTTP(w3, r3)
    if w3.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", w3.Code, w3.Body.String()) }
    var p3 struct{ Allowed bool `json:"allowed"` }
    if err := json.NewDecoder(bytes.NewReader(w3.Body.Bytes())).Decode(&p3); err != nil { t.Fatalf("decode p3: %v", err) }
    if !p3.Allowed { t.Fatalf("expected allowed=true for combined grants (no object_id)") }

    // Cleanup
    if err := auth.DeleteACLTuple(ctx, tenantID, "user", intr.UserID, perm, otype, &oid1); err != nil { t.Fatalf("delete obj-specific: %v", err) }
    if err := auth.DeleteACLTuple(ctx, tenantID, "user", intr.UserID, perm, otype, nil); err != nil { t.Fatalf("delete type-scoped: %v", err) }
}
