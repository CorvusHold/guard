...
func TestHTTP_Admin_Users_List_Update_BlockUnblock(t *testing.T) {
    ...
    if adminToks.AccessToken == "" { t.Fatalf("expected access token for admin") }
    ...
    if err := json.NewDecoder(bytes.NewReader(urec.Body.Bytes())).Decode(&userToks); err != nil { t.Fatalf("decode user tokens: %v", err) }
    aiUser, err := repo.GetAuthIdentityByEmailTenant(ctx, tenantID, userEmail)
    if err != nil { t.Fatalf("lookup user identity: %v", err) }
    ...
    if rec401.Code != http.StatusUnauthorized { t.Fatalf("expected 401, got %d: %s", rec401.Code, rec401.Body.String()) }
    ...
    if rec403.Code != http.StatusForbidden { t.Fatalf("expected 403, got %d: %s", rec403.Code, rec403.Body.String()) }
    ...
    if rec400.Code != http.StatusBadRequest { t.Fatalf("expected 400, got %d: %s", rec400.Code, rec400.Body.String()) }
    ...
    if rec200.Code != http.StatusOK { t.Fatalf("expected 200, got %d: %s", rec200.Code, rec200.Body.String()) }
    var list adminUsersResp
    if err := json.NewDecoder(bytes.NewReader(rec200.Body.Bytes())).Decode(&list); err != nil { t.Fatalf("decode list: %v", err) }
    if len(list.Users) < 2 { t.Fatalf("expected at least 2 users, got %d", len(list.Users)) }
    ...
    if recUN401.Code != http.StatusUnauthorized { t.Fatalf("expected 401, got %d: %s", recUN401.Code, recUN401.Body.String()) }
    ...
    if recUN403.Code != http.StatusForbidden { t.Fatalf("expected 403, got %d: %s", recUN403.Code, recUN403.Body.String()) }
    ...
    if recUN400.Code != http.StatusBadRequest { t.Fatalf("expected 400, got %d: %s", recUN400.Code, recUN400.Body.String()) }
    ...
    if recUNBad.Code != http.StatusBadRequest { t.Fatalf("expected 400, got %d: %s", recUNBad.Code, recUNBad.Body.String()) }
    ...
    if recUN204.Code != http.StatusNoContent { t.Fatalf("expected 204, got %d: %s", recUN204.Code, recUN204.Body.String()) }
    ...
    if recBlock.Code != http.StatusNoContent { t.Fatalf("expected 204, got %d: %s", recBlock.Code, recBlock.Body.String()) }
    u1, _ := repo.GetUserByID(ctx, aiUser.UserID)
    if u1.IsActive { t.Fatalf("expected user to be inactive after block") }
    ...
    if recUnblock.Code != http.StatusNoContent { t.Fatalf("expected 204, got %d: %s", recUnblock.Code, recUnblock.Body.String()) }
    u2, _ := repo.GetUserByID(ctx, aiUser.UserID)
    if !u2.IsActive { t.Fatalf("expected user to be active after unblock") }
}
