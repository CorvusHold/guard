...
func TestHTTP_SSO_WorkOS_StateReplay_401(t *testing.T) {
 ...
 c.RegisterV1(apiV1)
	...
	if recStart.Code != http.StatusFound {
		t.Fatalf("start not 302: %d", recStart.Code)
	}
	loc := recStart.Header().Get("Location")
	u, _ := url.Parse(loc)
	state := u.Query().Get("state")
	if state == "" {
		t.Fatalf("expected state from start")
	}
 ...
 req2.Header.Set("X-Auth-Mode", "bearer")
	...
	if !seen {
		t.Fatalf("expected auth.sso.login.failure invalid_state on replay")
	}
}
...
func TestHTTP_SSO_WorkOS_StateExpiry_401(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	if !ok {
		t.Fatalf("expected auth.sso.login.failure invalid_state after expiry")
	}
}
...
func TestHTTP_SSO_WorkOS_Callback_MissingState(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	if !found {
		t.Fatalf("expected auth.sso.login.failure missing_state")
	}
}
...
func TestHTTP_SSO_WorkOS_Callback_MissingCode_WithValidState(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	found := false
	for _, ev := range events {
		if ev.Type == "auth.sso.login.failure" && ev.Meta["reason"] == "code_required" {
			found = true
		}
	}
	if !found {
		t.Fatalf("expected auth.sso.login.failure code_required")
	}
}
...
func TestHTTP_SSO_WorkOS_Callback_TokenExchangeFailure(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	ok := false
	for _, ev := range events {
		if ev.Type == "auth.sso.login.failure" && ev.Meta["reason"] == "token_exchange_failed" {
			ok = true
		}
	}
	if !ok {
		t.Fatalf("expected auth.sso.login.failure token_exchange_failed")
	}
}
...
func TestHTTP_SSO_WorkOS_StartAndCallback(t *testing.T) {
 ...
 c.RegisterV1(apiV1)
	...
	if q.Get("response_type") != "code" {
		t.Fatalf("missing/invalid response_type: %s", q.Get("response_type"))
	}
	if q.Get("client_id") == "" {
	...
	if q.Get("connection") != "conn_123" {
		t.Fatalf("expected connection=conn_123, got %s", q.Get("connection"))
	}
	if q.Get("organization") != "org_456" {
		t.Fatalf("expected organization=org_456, got %s", q.Get("organization"))
	}
 ...
 req2.Header.Set("X-Auth-Mode", "bearer")
	...
	if claims["sub"] != ai.UserID.String() {
		t.Fatalf("sub mismatch: %v", claims["sub"])
	}
	if claims["ten"] != tenantID.String() {
		t.Fatalf("ten mismatch: %v", claims["ten"])
	}
	...
	if !ok {
		t.Fatalf("exp claim not number: %T", claims["exp"])
	}
	exp := int64(expF)
	...
	for _, e := range events {
		if e.Type == "auth.sso.login.success" && e.TenantID == tenantID && e.UserID == ai.UserID {
			if e.Meta["provider"] != "google" {
				t.Fatalf("provider mismatch: %v", e.Meta["provider"])
			}
			if e.Meta["email"] != "user.workos.itest@example.com" {
				t.Fatalf("email mismatch: %v", e.Meta["email"])
			}
			found = true
		}
	}
	if !found {
		t.Fatalf("expected auth.sso.login.success event for user")
	}
}
...
func TestHTTP_SSO_WorkOS_TokenExchange_TransportError_401(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	ok := false
	for _, ev := range events {
		if ev.Type == "auth.sso.login.failure" && ev.Meta["reason"] == "token_exchange_transport_error" {
			ok = true
		}
	}
	if !ok {
		t.Fatalf("expected auth.sso.login.failure token_exchange_transport_error")
	}
}
...
func TestHTTP_SSO_WorkOS_Callback_InvalidState_401(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	if !seen {
		t.Fatalf("expected auth.sso.login.failure invalid_state")
	}
}
...
func TestHTTP_SSO_WorkOS_ProfileMissingEmail_401(t *testing.T) {
 ...
 req.Header.Set("X-Auth-Mode", "bearer")
	...
	ok := false
	for _, ev := range events {
		if ev.Type == "auth.sso.login.failure" && ev.Meta["reason"] == "profile_missing_email" {
			ok = true
		}
	}
	if !ok {
		t.Fatalf("expected auth.sso.login.failure profile_missing_email")
	}
}
...
func TestHTTP_SSO_WorkOS_Start_UsesTenantDefaultsWhenMissing(t *testing.T) {
 ...
 c.RegisterV1(apiV1)
	...
	e.ServeHTTP(rec, req)
	if rec.Code != http.StatusFound {
		t.Fatalf("start not 302: %d", rec.Code)
	}
	loc := rec.Header().Get("Location")
	u, _ := url.Parse(loc)
	q := u.Query()
