...
test.describe('Admin FGA (Groups, Members, ACL)', () => {
  ...
  test('groups CRUD, membership add/remove, ACL create/delete', async ({
    page
  }) => {
    const TENANT = 'tenant_fga'
        ...
        page.route(
          ...
          async (route2) => {
            ...
            return route2.fulfill({
              status: 200,
              body: JSON.stringify({ groups: [created] }),
              headers: cors({ 'content-type': 'application/json' })
            })
          }
        )
    ...
    await expect(page.getByTestId('fga-groups')).toContainText('team-alpha', {
      timeout: 10000
    })
    ...
    await expect(page.getByTestId('fga-members-message')).toHaveText(/added/i, {
      timeout: 10000
    })

    await page.getByTestId('fga-member-remove').click()
    await expect(page.getByTestId('fga-members-message')).toHaveText(
      /removed/i,
      { timeout: 10000 }
    )
    ...
    await page.getByTestId('fga-acl-object-id').fill(TENANT)
    await page.getByTestId('fga-acl-create').click()
    await expect(page.getByTestId('fga-acl-message')).toHaveText(/created/i, {
      timeout: 10000
    })

    await page.getByTestId('fga-acl-delete').click()
    await expect(page.getByTestId('fga-acl-message')).toHaveText(/deleted/i, {
      timeout: 10000
    })
