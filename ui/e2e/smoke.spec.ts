...
test.describe('UI smoke', () => {
  test('admin page renders and stays open', async ({ page }) => {
    ...
    page.on('pageerror', (e) => console.log('PAGE ERROR', e?.message || String(e)));
    page.on('framenavigated', (fr) => console.log('NAVIGATED', fr.url()));

    // Use query param bootstrap to avoid any localStorage init
    await page.goto(`${UI_BASE}/admin?guard-base-url=${encodeURIComponent(UI_BASE)}&source=smoke`, { waitUntil: 'domcontentloaded' });
