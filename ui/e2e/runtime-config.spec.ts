...
test.describe('Runtime config bootstrap', () => {
  test('shows form when no config; saving persists and shows configured state', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('guard_runtime'));
  ...
  test('query params guard-base-url and source=redirect auto-persist and clean URL', async ({ page }) => {
    await clearStorage(page);
  ...
  test('uses existing localStorage config across reloads', async ({ page }) => {
    await clearStorage(page);
    // seed storage
    await page.evaluate((apiUrl) => localStorage.setItem('guard_runtime', JSON.stringify({ guard_base_url: apiUrl, source: 'direct' })), API_URL);
