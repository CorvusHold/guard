...
test.describe('Admin Settings', () => {
  ...
  test('load tenant settings then save update', async ({ page, browserName }, testInfo) => {
    ...
    await page.route(`**/v1/tenants/${TENANT}/settings`, async (route) => {
      ...
      getCalledResolve && getCalledResolve();
    ...
    await page.route(`**/v1/tenants/${TENANT}/settings`, async (route) => {
      const req = route.request();
      if (req.method() === 'OPTIONS') {
        return route.fulfill({ status: 204, headers: cors({ 'access-control-allow-methods': 'PUT,OPTIONS', 'access-control-allow-headers': 'content-type,authorization,accept,x-guard-client' }) });
      }
      if (req.method() === 'PUT') {
        putCalled = true;
        return route.fulfill({ status: 200, body: JSON.stringify({ ok: true }), headers: cors({ 'content-type': 'application/json' }) });
      }
      return route.fallback();
    });

    await page.goto(`${UI_BASE}/admin?guard-base-url=${encodeURIComponent(UI_BASE)}&source=test`, { waitUntil: 'domcontentloaded' });
    ...
    const viteOverlay = await page.evaluate(() => {
    ...
    await expect(page.getByTestId('admin-save-settings')).toBeEnabled({ timeout: 10000 }).catch(async (err) => {
      try { await page.screenshot({ path: testInfo.outputPath('admin-load-timeout.png'), fullPage: true }); } catch {}
      throw err;
    });
    ...
    if (errVisible) {
      const errText = await errLoc.textContent({ timeout: 100 }).catch(() => null);
    ...
    if (browserName !== 'webkit') {
      await expect(page.getByTestId('admin-workos-client-id')).toBeVisible({ timeout: 10000 });
      await expect(page.getByTestId('admin-workos-client-id')).toHaveValue('client_123', { timeout: 10000 });
      await expect(page.getByTestId('admin-workos-conn-id')).toHaveValue('conn_1', { timeout: 10000 });
      await expect(page.getByTestId('admin-workos-org-id')).toHaveValue('org_1', { timeout: 10000 });
      await expect(page.getByTestId('admin-sso-state-ttl')).toHaveValue('10m', { timeout: 10000 });
      await expect(page.getByTestId('admin-sso-redirect-allowlist')).toHaveValue('https://app.example.com/callback', { timeout: 10000 });
    } else {
      // Just ensure form is present
      await page.getByTestId('admin-workos-client-id').waitFor({ state: 'attached', timeout: 10000 });
    }
    ...
    await expect(page.getByTestId('admin-message')).toHaveText(/Settings saved/i);
    expect(putCalled).toBeTruthy();
  });

  test('portal link generation success displays link', async ({ page, browserName }, testInfo) => {
    if (browserName === 'webkit') test.slow();
    const TENANT = 'tenant_abc';
    const ORG = 'org_123';
    ...
    await page.route('**/v1/auth/sso/workos/portal-link**', async (route) => {
      ...
      if (!url.searchParams.get('tenant_id') || !url.searchParams.get('organization_id')) {
        return route.fulfill({ status: 400, body: JSON.stringify({ message: 'missing params' }), headers: cors({ 'content-type': 'application/json' }) });
      }
      return route.fulfill({ status: 200, body: JSON.stringify({ link: 'https://workos.example.com/portal/abc' }), headers: cors({ 'content-type': 'application/json' }) });
    });
    ...
    await page.getByTestId('admin-tenant-input').fill(TENANT);
    await expect(page.getByTestId('admin-tenant-input')).toHaveValue(TENANT);
    const viteOverlay2 = await page.evaluate(() => {
      const el = document.querySelector('#vite-error-overlay') as HTMLElement | null;
      return el ? el.innerText : null;
    }).catch(() => null as any);
    ...
    const errVisible2 = await errLoc2.isVisible({ timeout: 0 }).catch(() => false);
    if (errVisible2) {
      const errText2 = await errLoc2.textContent({ timeout: 100 }).catch(() => null);
    ...
    await expect(page.getByTestId('admin-portal-link-output')).toHaveText('https://workos.example.com/portal/abc', { timeout: 10000 });
  });
  ...
  test('portal link missing org shows client error', async ({ page }) => {
    ...
    await page.route(`**/v1/tenants/${TENANT}/settings`, async (route) => {
      ...
      return route.fulfill({ status: 200, body: JSON.stringify({ sso_provider: 'workos', workos_client_id: 'client_123', sso_state_ttl: '10m' }), headers: cors({ 'content-type': 'application/json' }) });
    });

    await page.goto(`/admin?guard-base-url=${encodeURIComponent(UI_BASE)}&source=redirect`);
    ...
    await expect(page.getByTestId('admin-save-settings')).toBeEnabled({ timeout: 10000 });

    await page.getByTestId('admin-generate-portal-link').click();
    await expect(page.getByTestId('admin-error')).toHaveText(/organization_id is required/i);
  });
});
