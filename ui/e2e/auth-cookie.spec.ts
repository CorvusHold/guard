...
test.describe('Cookie auth mode: guard + logout', () => {
  ...
  test('unauthenticated in cookie mode redirects /admin -> /', async ({
    ...
    await page.goto(
      `${UI_BASE}/admin?guard-base-url=${encodeURIComponent(UI_BASE)}&auth-mode=cookie&source=test`,
  ...
  test('logout in cookie mode calls API and redirects home', async ({
    ...
    await page.route('**/v1/auth/me', async (route) => {
      ...
      return route.fulfill({
        status: 200,
        body: JSON.stringify({ id: 'user_1', email: 'admin@example.com' }),
        headers: cors({ 'content-type': 'application/json' })
      })
    })
      ...
      .catch(async (err) => {
        try {
          await page.screenshot({
            path: testInfo.outputPath('cookie-admin-timeout.png'),
            fullPage: true
          })
        } catch {}
        throw err
