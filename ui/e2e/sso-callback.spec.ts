...
test.describe('SSO Callback', () => {
  ...
  test('workos callback success persists tokens and shows profile', async ({ page }) => {
    ...
    await page.route('**/v1/auth/me', async (route) => {
      ...
      const fulfilled = await route.fulfill({ status: 200, body: JSON.stringify({ email: 'user@example.com', first_name: 'Ada', last_name: 'Lovelace' }), headers: cors({ 'content-type': 'application/json' }) });
      meResolve && meResolve();
      return fulfilled;
    });

    await page.goto(`/auth/callback?guard-base-url=${encodeURIComponent(UI_BASE)}&source=redirect&provider=workos&code=abc123&state=xyz&email=${encodeURIComponent('user@example.com')}`);
  ...
  test('workos callback error shows error UI', async ({ page }) => {
    ...
    await page.route('**/v1/auth/sso/workos/callback**', async (route) => {
      ...
      const fulfilled = await route.fulfill({ status: 401, body: JSON.stringify({ message: 'invalid state' }), headers: cors({ 'content-type': 'application/json' }) });
      cbResolve && cbResolve();
      return fulfilled;
    });

    await page.goto(`/auth/callback?guard-base-url=${encodeURIComponent(UI_BASE)}&source=redirect&provider=workos&code=bad`);
    await page.waitForFunction(() => !!localStorage.getItem('guard_runtime'));
    await cbHit; // ensure callback processed
    ...
    await expect(page.getByTestId('callback-error')).toContainText(/invalid state/i, { timeout: 10000 });
  });

  test('unsupported provider yields error', async ({ page }) => {
    await page.goto(`/auth/callback?guard-base-url=${encodeURIComponent(UI_BASE)}&source=redirect&provider=foo&code=xyz`);
    await page.waitForFunction(() => !!localStorage.getItem('guard_runtime'));
    await expect(page.getByTestId('callback-error')).toBeVisible();
  ...
  test('missing code yields error', async ({ page }) => {
    await page.goto(`/auth/callback?guard-base-url=${encodeURIComponent(UI_BASE)}&source=redirect&provider=workos`);
    await page.waitForFunction(() => !!localStorage.getItem('guard_runtime'));
    await expect(page.getByTestId('callback-error')).toBeVisible();
