...
test.describe('Admin RBAC', () => {
  ...
  test('roles CRUD flow', async ({ page, browserName }, _testInfo) => {
            ...
            return route2.fulfill({
              ...
              body: JSON.stringify({
                roles: [
                  {
                    id: createdId,
                    name: body.name || 'roleA',
                    description: body.description || 'descA',
    ...
    await expect(page.getByTestId(`rbac-role-item-${createdId}`)).toBeVisible({
      timeout: 10000
    })

    // Update role name
    const nameInput = page
      .getByTestId(`rbac-role-item-${createdId}`)
      .locator('input')
      .nth(0)
    ...
    await page.route(
      ...
      async (route) => {
        ...
        return route.fulfill({
          status: 200,
          body: JSON.stringify({
            roles: [{ id: 'role_1', name: 'role1', tenant_id: TENANT }]
          }),
          headers: cors({ 'content-type': 'application/json' })
        })
      }
  ...
  test('user roles list, add and remove', async ({ page }) => {
        ...
        if (req.method() === 'POST') {
          ...
          page.route(
            ...
            async (route2) => {
              ...
              return route2.fulfill({
                status: 200,
                body: JSON.stringify({ role_ids: [body.role_id || 'role_1'] }),
                headers: cors({ 'content-type': 'application/json' })
              })
            }
          )
    ...
    await expect(page.getByTestId('rbac-user-role-item-role_1')).toBeVisible({
      timeout: 10000
    })
  ...
  test('permissions viewer lists permissions', async ({ page }) => {
    ...
    await expect(page.getByTestId('rbac-permissions-item-perm_1')).toBeVisible({
      timeout: 10000
    })
    await expect(page.getByTestId('rbac-permissions-item-perm_2')).toBeVisible({
      timeout: 10000
    })
  })
})
