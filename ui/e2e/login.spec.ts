...
function absApi(path: string): RegExp {
  const p = path.startsWith('/') ? path : `/${path}`;
  return new RegExp(`^https?:\/\/[^/]+${rxEscape(p)}(\\?.*)?$`);
}

function matchPath(u: unknown, path: string): boolean {
  try {
    if (typeof u === 'string') {
      return new URL(u).pathname === path;
    }
    const maybe = u as { pathname?: string };
    if (maybe && typeof maybe.pathname === 'string') return maybe.pathname === path;
  } catch {
...
async function setupRuntime(page: import('@playwright/test').Page) {
  await page.goto(`/?guard-base-url=${encodeURIComponent(API_URL)}&source=redirect`);
...
async function routePasswordLoginSuccess(page: import('@playwright/test').Page): Promise<void> {
  await page.route('**/v1/auth/password/login', async (route) => {
    ...
    if (body && body.email && body.password && !body.tenant_id) {
      console.log('ROUTE FULFILL /v1/auth/password/login 400 tenant required');
      return route.fulfill({ status: 400, body: JSON.stringify({ message: 'tenant required' }), headers: cors({ 'content-type': 'application/json' }) });
    }
  ...
  await page.route('**/v1/auth/me', async (route) => {
    ...
    return route.fulfill({
      status: 200,
      body: JSON.stringify({ email: 'user@example.com', first_name: 'Ada', last_name: 'Lovelace' }),
      headers: cors({ 'content-type': 'application/json' }),
    });
  });
}
...
test.describe('Login flow', () => {
  ...
  test('tenant required then success after providing tenant', async ({ page }) => {
    await setupRuntime(page);
  ...
  test('MFA challenge (202) shows banner', async ({ page }) => {
    ...
    await page.route('**/v1/auth/password/login', async (route) => {
      const req = route.request();
      console.log('ROUTE HIT /v1/auth/password/login (MFA 202)', req.method(), req.url());
      if (req.method() === 'OPTIONS') {
        console.log('ROUTE FULFILL /v1/auth/password/login OPTIONS 204');
        return route.fulfill({
          status: 204,
          headers: cors({ 'access-control-allow-methods': 'POST,GET,OPTIONS', 'access-control-allow-headers': 'content-type,authorization,accept,x-guard-client' }),
        });
      }
  ...
  test('MFA verify flow completes login', async ({ page }) => {
    ...
    const loginHit2 = new Promise<void>((r) => (loginResolve2 = r));
    await page.route('**/v1/auth/password/login', async (route) => {
      const req = route.request();
      if (req.method() === 'OPTIONS') {
        return route.fulfill({
    ...
    await page.route('**/v1/auth/mfa/verify', async (route) => {
      ...
      if (req.method() === 'OPTIONS') {
        ...
        return route.fulfill({
          status: 204,
          headers: cors({ 'access-control-allow-methods': 'POST,OPTIONS', 'access-control-allow-headers': 'content-type,authorization,accept,x-guard-client' }),
        });
      }
      const body = req.postDataJSON?.() as any;
      if (body?.challenge_token === 'mfa-123' && body?.code === '123456') {
        console.log('ROUTE FULFILL /v1/auth/mfa/verify 200');
        const fulfilled = await route.fulfill({ status: 200, body: JSON.stringify({ access_token: 'acc', refresh_token: 'ref' }), headers: cors({ 'content-type': 'application/json' }) });
        mfaResolve && mfaResolve();
        return fulfilled;
      }
      console.log('ROUTE FULFILL /v1/auth/mfa/verify 400');
      return route.fulfill({ status: 400, body: JSON.stringify({ message: 'invalid code' }), headers: cors({ 'content-type': 'application/json' }) });
    });
    ...
    await page.route('**/v1/auth/me', async (route) => {
      ...
      console.log('ROUTE FULFILL /v1/auth/me 200');
      const fulfilled = await route.fulfill({ status: 200, body: JSON.stringify({ email: 'user@example.com', first_name: 'Grace', last_name: 'Hopper' }), headers: cors({ 'content-type': 'application/json' }) });
      meResolve && meResolve();
      return fulfilled;
    });
    ...
    await Promise.all([
      mfaHit,
      page.getByTestId('mfa-verify').click(),
    ]);
    await meHit;
  ...
  test('Tenant discovery -> select tenant -> login success', async ({ page }) => {
    ...
    await page.route('**/v1/auth/tenants*', async (route) => {
      ...
      const fulfilled = await route.fulfill({ status: 200, body: JSON.stringify({ tenants: email ? [{ id: 'tenant_1', name: 'Acme' }, { id: 'tenant_2', name: 'Beta' }] : [] }), headers: cors({ 'content-type': 'application/json' }) });
      tenantsResolve && tenantsResolve();
      return fulfilled;
    });
    ...
    await Promise.all([
      tenantsHit,
      page.getByTestId('login-discover').click(),
    ]);
  ...
  test('SSO start (dev) navigates to redirect_url', async ({ page }) => {
    ...
    await page.route('**/v1/auth/sso/dev/start*', async (route) => {
      ...
      if (req.method() === 'OPTIONS') {
        ...
        return route.fulfill({
          status: 204,
          headers: cors({ 'access-control-allow-methods': 'GET,OPTIONS', 'access-control-allow-headers': 'content-type,authorization,accept,x-guard-client' }),
        });
      }
      console.log('ROUTE FULFILL /v1/auth/sso/dev/start 200 (Location header)');
      const fulfilled = await route.fulfill({ status: 200, headers: cors({ location: 'https://example.com/sso-dev' }) });
      ssoResolve && ssoResolve();
      return fulfilled;
    });

    await Promise.all([
      ssoHit,
      page.getByTestId('login-sso-dev').click(),
    ]);
