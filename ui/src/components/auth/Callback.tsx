  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const res = await client.handleSsoCallback(provider as any, { code, state, email });
  ...
  return (
    <div className="flex min-h-svh flex-col items-center justify-center p-6">
      <div className="w-full max-w-lg space-y-4 rounded-xl border p-6">
        ...
        {status === "loading" && (
          <div className="text-sm text-muted-foreground">Processing callback with {provider.toUpperCase()}...</div>
        )}
        {status === "success" && (
          <div className="space-y-2">
            <div className="rounded-md border p-3 text-sm">
              ...
              {profile && (
                <div className="text-xs text-muted-foreground">Signed in as {(profile as any).email}</div>
              )}
            </div>
        ...
        {status === "error" && (
          <div className="space-y-2">
            <div className="rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700" data-testid="callback-error">
              {message}
            </div>
            <div className="flex gap-2">
