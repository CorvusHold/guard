...
export default function RolesPanel({ tenantId }: { tenantId: string }) {
  ...
  const [loading, setLoading] = useState<
    null | 'list' | 'create' | 'update' | 'delete'
  >(null)
  ...
  async function createRole() {
    ...
    setLoading('create')
    try {
      const res = await getClient().rbacCreateRole({
        tenant_id: tenantId,
        name: newName.trim(),
  ...
  async function updateRole(
    ...
    setLoading('update')
    try {
      const res = await getClient().rbacUpdateRole(r.id, {
        tenant_id: tenantId,
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-md border p-3 space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <input
            data-testid="rbac-role-new-name"
          ...
          />
          <input
            data-testid="rbac-role-new-desc"
            className="w-full rounded-md border px-3 py-2 text-sm"
          ...
          <div className="space-y-2">
            {roles.map((r) => (
              <div
                ...
                data-testid={`rbac-role-item-${r.id}`}
              >
                ...
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    if (v !== r.name) void updateRole(r, { name: v })
                  }}
                />
                <input
                  className="w-full rounded-md border px-2 py-1 text-sm"
                  ...
                  onBlur={(e) => {
                    const v = e.target.value
                    if (v !== (r.description || ''))
                      void updateRole(r, { description: v })
                  }}
                />
                <div className="flex gap-2">
                  <Button
                    ...
                    data-testid={`rbac-role-delete-${r.id}`}
                    disabled={loading !== null}
                  >
                    Delete
                  </Button>
