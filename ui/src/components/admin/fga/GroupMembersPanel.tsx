...
export default function GroupMembersPanel({
  groupId
}: GroupMembersPanelProps): React.JSX.Element {
  ...
  async function onAdd() {
    ...
    setError(null)
    if (!groupId || !userId) {
      setError('groupId and userId are required')
      return
    }
    setLoading('add')
    try {
  ...
  return (
    <div className="space-y-3" data-testid="fga-members">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
        ...
        />
        <Button
          data-testid="fga-member-add"
          onClick={() => onAdd()}
          disabled={!userId || !groupId || loading !== null}
        >
          {loading === 'add' ? 'Adding...' : 'Add Member'}
        </Button>
        <Button
          data-testid="fga-member-remove"
          variant="destructive"
          onClick={() => onRemove()}
          disabled={!userId || !groupId || loading !== null}
        >
      ...
      {message && (
        <div
          ...
          data-testid="fga-members-message"
        >
          {message}
        </div>
      )}
