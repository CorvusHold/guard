...
export default function ACLPanel({
  tenantId
}: ACLPanelProps): React.JSX.Element {
  ...
  async function createTuple() {
    ...
    setError(null)
    if (!canSubmit) {
      setError('All required fields must be set')
      return
    }
    setLoading('create')
    try {
    ...
    <div className="space-y-3" data-testid="fga-acl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-6 gap-2">
        <select
          ...
          value={subjectType}
          onChange={(e) =>
            setSubjectType((e.target.value as 'user' | 'group') || 'user')
          }
        >
      ...
      {message && (
        <div
          ...
          data-testid="fga-acl-message"
        >
          {message}
        </div>
      )}
      <div className="text-xs text-muted-foreground">
        Note: Tenant ID is taken from the field at the top of this page.
      </div>
    </div>
  )
}
