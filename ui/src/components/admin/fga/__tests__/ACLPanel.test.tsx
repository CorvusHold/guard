...
vi.mock('@/lib/sdk', () => ({
  getClient: () => ({
    fgaCreateAclTuple: createAcl,
    fgaDeleteAclTuple: deleteAcl
  })
}))
...
describe('ACLPanel', () => {
  ...
  it('creates and deletes ACL tuple (happy path)', async () => {
    ...
    fireEvent.change(screen.getByTestId('fga-acl-subject-id'), {
      target: { value: 'u1' }
    })
    fireEvent.change(screen.getByTestId('fga-acl-permission-key'), {
      target: { value: 'users.read' }
    })
    fireEvent.change(screen.getByTestId('fga-acl-object-type'), {
      target: { value: 'tenant' }
    })
    fireEvent.change(screen.getByTestId('fga-acl-object-id'), {
      target: { value: 'ten1' }
    })
  ...
  it('handles error from create', async () => {
    ...
    fireEvent.click(screen.getByTestId('fga-acl-create'))
    await waitFor(() =>
      expect(screen.getByTestId('fga-acl-error')).toBeInTheDocument()
    )
  })
})
