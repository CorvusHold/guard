...
describe('MyMfaPanel', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    startTotp.mockResolvedValue({
      meta: { status: 200 },
      data: { secret: 'S', otpauth_url: 'otpauth://test' }
    })
    activateTotp.mockResolvedValue({ meta: { status: 200 } })
    disableTotp.mockResolvedValue({ meta: { status: 200 } })
    generateBackup.mockResolvedValue({
      meta: { status: 200 },
      data: { codes: ['A', 'B'] }
    })
  ...
  it('happy path flows', async () => {
    ...
    // Activate TOTP
    fireEvent.change(screen.getByTestId('mfa-code'), {
      target: { value: '123456' }
    })
  ...
  it('error path for startTOTP shows banner', async () => {
    ...
    fireEvent.click(screen.getByTestId('mfa-start'))
    await waitFor(() =>
      expect(screen.getByTestId('my-mfa-error')).toBeInTheDocument()
    )
  })
})
