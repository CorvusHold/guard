...
export default function MyMfaPanel(): React.JSX.Element {
  ...
  return (
    <div className="space-y-3" data-testid="my-mfa">
      ...
      {error && (<div className="rounded-md border border-red-200 bg-red-50 p-2 text-sm text-red-700" data-testid="my-mfa-error">{error}</div>)}
      {message && (<div className="rounded-md border border-green-200 bg-green-50 p-2 text-sm text-green-800" data-testid="my-mfa-message">{message}</div>)}

      <div className="space-y-2">
        <div className="flex gap-2 flex-wrap">
          <Button data-testid="mfa-start" onClick={() => startTotp()} disabled={loading !== null}>{loading === 'start' ? 'Starting...' : 'Start TOTP'}</Button>
          <input data-testid="mfa-code" className="rounded-md border px-3 py-2 text-sm" placeholder="123456" value={code} onChange={(e) => setCode(e.target.value)} />
          <Button data-testid="mfa-activate" onClick={() => activateTotp()} disabled={!code || loading !== null}>{loading === 'activate' ? 'Activating...' : 'Activate'}</Button>
          <Button data-testid="mfa-disable" variant="secondary" onClick={() => disableTotp()} disabled={loading !== null}>{loading === 'disable' ? 'Disabling...' : 'Disable'}</Button>
        </div>
        {(secret || otpauth) && (
          <div className="rounded-md border p-2 text-sm">
            {secret && (<div><span className="font-medium">Secret:</span> <code data-testid="mfa-secret">{secret}</code></div>)}
            {otpauth && (<div className="mt-1 break-all"><span className="font-medium">otpauth:</span> <span data-testid="mfa-otpauth">{otpauth}</span></div>)}
          </div>
        )}
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Button data-testid="mfa-generate-backup" onClick={() => generateBackup()} disabled={loading !== null}>{loading === 'generate' ? 'Generating...' : 'Generate Backup Codes'}</Button>
          <Button data-testid="mfa-refresh-count" variant="secondary" onClick={() => refreshCount()} disabled={loading !== null}>{loading === 'count' ? 'Refreshing...' : 'Refresh Count'}</Button>
          <div className="text-sm text-muted-foreground">Remaining: <span data-testid="mfa-count">{count ?? '-'}</span></div>
        </div>
      </div>
    </div>
  );
