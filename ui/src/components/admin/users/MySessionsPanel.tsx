...
export default function MySessionsPanel(): React.JSX.Element {
  ...
  return (
    <div className="space-y-3">
      ...
      {error && (
        <div className="rounded-md border border-red-200 bg-red-50 p-2 text-sm text-red-700" data-testid="sessions-error">{error}</div>
      )}
            ...
            <tbody>
              {sessions.map((s) => (
                <tr key={s.id} className="border-b last:border-b-0">
                  ...
                  <td className="py-2 pr-3">{s.created_at ? new Date(s.created_at).toLocaleString() : '-'}</td>
                  <td className="py-2 pr-3">{s.expires_at ? new Date(s.expires_at).toLocaleString() : '-'}</td>
                  <td className="py-2 pr-3">
                    <span className={s.revoked ? 'text-red-600' : 'text-green-600'}>
                      {s.revoked ? 'revoked' : 'active'}
                    </span>
                  </td>
                  <td className="py-2 pr-3">
                    <Button variant="destructive" size="sm" disabled={s.revoked} onClick={() => revoke(s.id)}>Revoke</Button>
                  </td>
                </tr>
              ))}
            </tbody>
